      ...
      <CardBody>
        ...
        <button
          ...
          className={`w-full inline-flex items-center justify-center gap-1.5 rounded-md border px-3 py-2
            ...
            ${done ? 'border-success-100 bg-success-50 text-success-600'
                   : 'border-brand-200 bg-brand-50 text-brand-700 hover:bg-brand-100'}`}
        >
          {done ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
          {done ? 'Copied for the console' : 'Copy price and date'}
        </button>
        <p className="text-[10px] text-ink-400 mt-2">
          Paste into the LabStack console and mark the request quoted. Once the store accepts,
          this appears in the Network bucket with a clock on it.
        </p>
      </CardBody>
    </Card>
  );
